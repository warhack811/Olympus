...
def is_rag_v2_enabled_for_user(user_id: int | None) -> bool:
    """
    RAG v2 kullanıcı bazlı kapı.
    - Global flag kapalıysa -> False
    - Test user listesi varsa sadece onlar -> True
    """
    if not (feature_enabled("rag_v2_enabled", default=False) or feature_enabled("rag_v2", default=False)):
        return False

    if not user_id:
        return False

    test_users = _flags_cache.get("rag_v2_test_user_ids")
    if not test_users:
        return True  # herkese açık

    if isinstance(test_users, str):
        allowed = {u.strip() for u in test_users.split(",")}
    else:
        allowed = set(test_users)
