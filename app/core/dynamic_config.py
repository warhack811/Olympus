...
class DynamicConfigService:
    ...
    def get(self, key: str, default: T = None, use_cache: bool = True) -> T | Any:
        """
    ...
    def get_model(self, purpose: str, provider: str | None = None) -> dict[str, Any] | None:
        """
        ...
        try:
            with get_session() as session:
                query = select(ModelConfig).where(ModelConfig.purpose == purpose, ModelConfig.is_active)
    ...
    def get_all_models(self, active_only: bool = True) -> list[dict[str, Any]]:
        ...
        try:
            with get_session() as session:
                ...
                if active_only:
                    query = query.where(ModelConfig.is_active)
    ...
    def get_active_theme(self) -> dict[str, Any] | None:
        ...
        try:
            with get_session() as session:
                theme = session.exec(select(ThemeConfig).where(ThemeConfig.is_active, ThemeConfig.is_default)).first()

                if not theme:
                    # Default yoksa ilk aktifi al
                    theme = session.exec(
                        select(ThemeConfig).where(ThemeConfig.is_active).order_by(ThemeConfig.sort_order)
                    ).first()
    ...
    def get_all_themes(self, active_only: bool = True) -> list[dict[str, Any]]:
        ...
        try:
            with get_session() as session:
                ...
                if active_only:
                    query = query.where(ThemeConfig.is_active)
        ...
        try:
            with get_session() as session:
                persona = session.exec(
                    select(PersonaConfig).where(PersonaConfig.name == name, PersonaConfig.is_active)
                ).first()
    ...
    def get_all_personas(self, active_only: bool = True) -> list[dict[str, Any]]:
        ...
        try:
            with get_session() as session:
                ...
                if active_only:
                    query = query.where(PersonaConfig.is_active)
