    ...
    def check_limits_pre_flight(user_id: int) -> None:
        ...
        with get_session() as session:
            ...
            if count_last_min >= LIMIT_CHAT_PER_MINUTE:
                logger.warning(
                    f"[LIMITER] Dakikalık limit aşıldı: user={user_id} count={count_last_min}/{LIMIT_CHAT_PER_MINUTE}"
                )
                raise HTTPException(
        ...
        if counter.total_chat_count >= total_limit:
            logger.warning(
                f"[LIMITER] Günlük limit aşıldı: user={user_id} count={counter.total_chat_count}/{total_limit}"
            )
            raise HTTPException(
                status_code=status.HTTP_429_TOO_MANY_REQUESTS,
