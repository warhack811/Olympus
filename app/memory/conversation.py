...
def _default_user_resolver(username: str) -> int | None:
    """Varsayılan user resolver (hata verir)."""
    logger.error("[CONV_STORE] User ID resolver henüz set edilmedi!")
    return None


_user_id_resolver: Callable[[str], int | None] = _default_user_resolver


def set_user_resolver(resolver_func: Callable[[str], int | None]) -> None:
...
def create_conversation(
    username: str, first_message: str | None = None, title: str | None = None, preset_id: int | None = None
):
    """
...
def get_conversation(username: str, conv_id: str):
    """
    Sohbet detaylarını döndürür.
    ...
    Returns:
        Conversation veya None: Sohbet nesnesi
    """
    get_session, Conversation, _ = _get_imports()
    user_id = _resolve_user_id(username)
    ...
    with get_session() as session:
        ...
        if not conv or conv.user_id != user_id:
            logger.warning(f"[CONV] Yetkisiz erişim veya bulunamadı: {username} -> {conv_id}")
            return None
        return conv
...
def get_conversation_summary_text(conv_id: str) -> str | None:
    """
...
def get_recent_context(username: str, conv_id: str, max_messages: int = 4) -> str | None:
    """
