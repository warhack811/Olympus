...
async def add_memory(
    ...
    Args:
        ...
        importance: Önem skoru (0.0-1.0)
        tags: Etiketler
        memory_type: Hafıza tipi (fact, event, state)
        topic: Konu başlığı (opsiyonel, tags[0]'dan daha öncelikli)
    ...
    # Topic resolution: Explicit arg > First tag > Default
    final_topic = topic
    if not final_topic:
        final_topic = tags[0] if tags and len(tags) > 0 else "general"

    try:
        record = await MemoryService.add_memory(
            user_id=user_id, text=text, importance=importance, topic=final_topic, memory_type=memory_type
        )
...
async def update_memory(username: str, memory_id: str, text: str, importance: float | None = None) -> MemoryItem | None:
    """
