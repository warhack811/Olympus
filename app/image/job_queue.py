...
class ImageJob:
    def __init__(
        ...
        message_id: int | None = None,  # Güncellenecek mesaj ID'si
        image_settings: dict | None = None,  # Custom image settings
    ):
...
class ImageJobQueue:
    ...
    def _ensure_worker_started(self):
        ...
        print(f"[DEBUG_PRINT] _ensure_worker_started called, _started={self._started}")
        if not self._started:
            try:
                loop = asyncio.get_running_loop()
                self._worker_task = loop.create_task(self._worker_loop())
                self._started = True
                print("[DEBUG_PRINT] Worker task CREATED")
                logger.info("[IMAGE_QUEUE] Worker başlatıldı")
            except RuntimeError as e:
                print(f"[DEBUG_PRINT] Worker start FAILED: {e}")
                # Event loop yok - startup event'te başlatılacak
                pass

    # ---------- ASYNC WORKER ----------
    async def _worker_loop(self) -> None:
        print("[DEBUG_PRINT] _worker_loop STARTED - waiting for jobs")
        while True:
            job = await self._queue.get()
            print(f"[DEBUG_PRINT] Worker got job: {job.job_id[:8]}")
