    ...
    async def execute(
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                last_error = e
                logger.warning(
                    f"[SAFE_CALLBACK] Job {job_id} callback failed (attempt {attempt + 1}/{self.max_retries}): {e}"
                )
        ...
        logger.error(
            f"[SAFE_CALLBACK] Job {job_id} callback failed after {self.max_retries} attempts. Last error: {last_error}"
        )
