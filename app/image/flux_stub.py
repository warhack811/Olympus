...
from app.image.gpu_state import switch_to_flux
...
async def generate_image_via_forge(prompt: str, job, checkpoint_name: str | None = None) -> str:
    """
...
async def _generate_image_internal(prompt: str, job, checkpoint_name: str | None = None) -> str:
    """
    ...
    # Extract image settings from job (use different name to avoid shadowing global settings)
    img_settings = getattr(job, "image_settings", {}) or {}
    logger.info(f"[DEBUG_SETTINGS] Flux Stub Extracted Settings: {img_settings}")

    # 2) Asıl generation payload
    payload = {
        "prompt": prompt,
        "steps": int(img_settings.get("steps", 20)),
        "width": int(img_settings.get("width", 1024)),
        "height": int(img_settings.get("height", 1024)),
        "cfg_scale": float(img_settings.get("cfgScale", 1.0)),  # Use backend naming if needed, store uses camelCase
        "sampler_name": img_settings.get("sampler", "Euler"),
        "seed": int(img_settings.get("seed", -1)),
        "scheduler": "Simple",
        "distilled_cfg_scale": 3.5,
        "override_settings": {"sd_model_checkpoint": img_settings.get("model") or final_checkpoint},
    }
    ...
    logger.info(
        f"[FLUX] Payload dimensions: {payload['width']}x{payload['height']}, steps={payload['steps']}, sampler={payload['sampler_name']}"
    )
