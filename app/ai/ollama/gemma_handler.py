...
async def run_local_chat(
    ...
    if not system_prompt:
        ...
        system_prompt = "Sen Mami AI'sın. Türkçe konuş, sansürsüz ol."
    ...
    def _clean_message_content(text: str) -> str:
        ...
        text = re.sub(r"^\s*\[(GROQ|BELA|LOCAL|GROÇ)\]\s*", "", text, flags=re.IGNORECASE)
        text = re.sub(r"\[(GROQ|BELA|LOCAL|GROÇ)\]\s*", "", text, flags=re.IGNORECASE)
        return text.strip()
    ...
    try:
        ...
        return "(LOCAL) Çok düşündüm ama toparlayamadım. Tekrar eder misin?"
    except Exception as e:
        logger.error(f"[LOCAL_CHAT] Bağlantı hatası: {e}")
        return "(LOCAL) Beynim (GPU) şu an yanıt vermiyor. Lütfen Ollama'yı kontrol et."
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=120.0) as client:
            async with client.stream("POST", f"{base_url}/api/chat", json=payload) as resp:
                ...
                # YENİ HİBRİT YAKLAŞIM (REAL-TIME STREAMING)
                # Düşünce bloklarını anlık filtreleyerek kelime kelime gönder
                in_think = False
                ...
                async for line in resp.aiter_lines():
                    ...
                    if content:
                        ...
                        cleaned_chunk = strip_think_blocks(cleaned_chunk)  # Extra safety if regex leaks
        ...
        yield "(LOCAL) Zaman aşımı: Ollama yanıt vermedi."
    except Exception as e:
        ...
        yield f"(LOCAL) Beynim (GPU) şu an yanıt vermiyor. (Hata: {str(e)})"
