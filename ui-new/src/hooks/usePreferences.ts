...
export function usePreferences() {
    ...
    useEffect(() => {
        if (preferencesQuery.data) {
            ...
            if (prefs.imageSize) setImageSettings({ aspectRatio: prefs.imageSize as ImageSettings['aspectRatio'] })
    ...
    const saveImageSettings = useCallback((settings: Partial<ImageSettings>) => {
        ...
        if (settings.aspectRatio) savePreferenceMutation.mutate({ key: 'imageSize', value: settings.aspectRatio })
