...
export function WelcomeScreen() {
    ...
    return (
        <motion.div
            ...
            className="flex flex-col items-center justify-center h-full text-center px-4 py-1 md:px-6 md:py-12"
        >
            {/* Animated Logo */}
            <motion.div
                className="relative w-16 h-16 md:w-28 md:h-28 mb-4 md:mb-8"
                animate={{
                    scale: [1, 1.05, 1],
                    rotate: [0, 3, -3, 0]
                }}
            ...
            <h1 className="text-2xl md:text-4xl font-display font-bold mb-2 md:mb-3">
                <span className="gradient-text">{greeting}</span>
                <span className="text-(--color-text)">, {displayName}!</span>
            </h1>

            <p className="text-(--color-text-secondary) text-sm md:text-lg max-w-md mb-6 md:mb-10 leading-relaxed hidden md:block">
                Ben {branding.displayName}, kişisel asistanınız. Size nasıl yardımcı olabilirim?
            </p>
            <p className="text-(--color-text-secondary) text-sm md:text-lg max-w-md mb-4 md:mb-10 leading-relaxed md:hidden">
                Size nasıl yardımcı olabilirim?
            </p>

            {/* Quick Action Cards */}
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 md:gap-4 max-w-3xl w-full">
                <QuickActionCard
            ...
            {/* Example Prompts */}
            <div className="mt-4 md:mt-12 w-full max-w-2xl">
                <p className="text-xs md:text-sm text-(--color-text-muted) mb-2 md:mb-4">Örnek istekler:</p>
                <div className="grid grid-cols-2 md:flex md:flex-wrap md:justify-center gap-2">
                    {examplePrompts.map((prompt, i) => (
                        <button
                            ...
                            className={cn(
                                "h-auto px-2 py-2 md:px-4 md:py-2 rounded-xl md:rounded-full text-xs md:text-sm text-left md:text-center whitespace-normal leading-tight",
    ...
    return (
        ...
        >
            ...
            <div className="relative">
                ...
                <h3 className="font-semibold text-xs md:text-base text-(--color-text) mb-0.5 md:mb-1 leading-tight">{title}</h3>
                <p className="text-sm text-(--color-text-muted) hidden md:block">{description}</p>
            </div>
